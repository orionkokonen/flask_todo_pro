...
このリビジョンは初期スキーマ定義のみを扱い、データ移行は含まない。
"""
...
def upgrade():
    """初期テーブル群を作成して、アプリが動ける土台を用意する。"""
    # 初回セットアップ時に必要なテーブル/インデックスを作成する。
...
def downgrade():
    """upgrade() で作った土台を逆順で取り除き、元に戻せるようにする。"""
    # upgrade で作成した構造を逆順で戻せるようにしている。
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table('subtask', schema=None) as batch_op:
