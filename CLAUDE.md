...
| #   | ファイル                    | 役割                               |
| --- | --------------------------- | ---------------------------------- |
| 1   | app/**init**.py             | アプリの組み立て工場（ファクトリ） |
...
| 7   | app/auth/**init**.py        | 認証Blueprintの入口                |
| 8   | app/auth/routes.py          | ログイン・登録・ログアウトの処理   |
| 9   | app/todo/**init**.py        | Todo Blueprintの入口               |
...
※ **init**.py（7, 9番）と conftest.py（テスト1番）は数行の入口ファイルなので、実質読むべきは23ファイル
...
| .gitignore               | Gitで追跡しないファイルの一覧（.env、**pycache**等）        |
