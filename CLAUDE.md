...
3. 全ルート共通の「お決まりの作法」
   8本全部、頭に同じパターンが書いてあります。 task_new を例に(app/todo/routes_tasks.py:51-53):

@bp.route("/tasks/new", methods=["GET", "POST"])
@login_required # ← ① 未ログインならログイン画面へ
def task_new():
...
task_detail も同じく(app/todo/routes_tasks.py:119-124):

@login_required
def task_detail(task_id: int):
task = get_or_404(Task, task_id)
ensure_task_access(task) # ← ② 他人のタスクなら 403 で拒否
つまりどのルートも:

@login_required … ログインしてる? (=認証)
ensure_task_access(task) … そのタスクを触ってよい人? (=認可)
の2段チェックを必ず通します。

面接ワード: 「認証(あなたは誰?) と 認可(あなたに何が許される?) を全ルートで2段で通します」
...
5. ★4本目★ レート制限 — ブルートフォース対策
   コード — app/security.py:21-34

class SimpleRateLimiter:
def **init**(self) -> None:
self.\_entries: dict[str, deque[float]] = {}
self.\_lock = Lock()
{"login:127.0.0.1": [失敗時刻, 失敗時刻, ...]} の形でIPごとに失敗時刻を記録。シングルトン化 (app/security.py:107)。

スライディングウィンドウ方式
「直近N秒にM回まで」。時間枠が時間と一緒にスライドする。

app/security.py:36-52 の \_prune で時間枠外の古い記録を捨てる。

cutoff = now - window_seconds
while entries and entries[0] <= cutoff:
entries.popleft()
失敗時だけカウント / 成功時にリセット
record_failure (app/security.py:77-89): 失敗時のみカウント。正常ユーザーは巻き込まない
reset (app/security.py:91-97): 成功時にゼロクリア。正規ユーザーが過去の失敗で詰むのを防ぐ
固定ウィンドウとの違い (面接でよく聞かれる)
固定: 「毎分10回まで」を毎分0秒リセット → 0:59に10回 + 1:00に10回で1秒に20回試せる抜け穴あり
スライディング: 「直近60秒に10回まで」 → 境界がないので抜け道なし
制約
app/security.py:10-11 の通り、メモリ上なので複数プロセスでは共有できない。本番ならRedisベースのFlask-Limiterに置き換える、と言えると強い。

面接ワード
「スライディングウィンドウで、IPごとにログイン失敗時刻をdequeで持って閾値超過で一時ブロック。失敗時だけカウント、成功時reset で正規ユーザーは詰まりません」
...
Q3. レート制限は固定ウィンドウじゃダメ?
境界またぎの集中攻撃に弱いから。「毎分10回まで」だと0:59と1:00で1秒に20回試せる。スライディングなら境界の概念がないので抜け穴がない。
