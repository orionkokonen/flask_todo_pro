...
├── .mypy_cache/                  ← ⚠️型チェックツール(mypy)のキャッシュ（Git管理外）
├── .pytest_cache/                ← pytestキャッシュ（Git管理外）
├── .venv_work/                   ← Python仮想環境（Git管理外）
├── instance/                     ← ⚠️SQLiteのDBファイルなどローカルデータ（Git管理外）
├── .env                          ← 秘密情報の実体（Git管理外）
└── *.db                          ← ⚠️SQLiteデータベースファイル（Git管理外）
```

⚠️は、すべて **「条件を満たしたときだけ自動生成される」** ファイル/フォルダなので、今のPC上にまだ作られていない。

| ファイル/フォルダ | いつ生成されるか                         | 今ない理由                                                       |
| ----------------- | ---------------------------------------- | ---------------------------------------------------------------- |
| `.mypy_cache/`    | `mypy`（型チェックツール）を実行したとき | `mypy` をまだ実行していない                                      |
| `instance/`       | FlaskがSQLiteを使うとき自動生成          | 本番はPostgreSQLなので、ローカルでSQLite起動していなければ出ない |
| `*.db`            | SQLiteのデータベースファイル             | 同上                                                             |

つまり、**ファイルが消えたわけではなく、まだ一度も生成されていないだけ** 。  
`.gitignore` には、**「もし作られたらGitに入れないでね」** というルールとして書いてある。
