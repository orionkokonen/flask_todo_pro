...
- ユニット1
  なぜ create_app() という関数にしているか?
  → ファクトリパターン。テストで設定を差し替えて新しいアプリを作れるから。

なぜ db = SQLAlchemy() を関数の外で作り、db.init_app(app) で後から紐づけるのか?
→ 他ファイル(models.py等)から import できるようにするため。アプリへの紐付けは起動時に決まる。
