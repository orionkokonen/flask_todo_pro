...
def test_team_detail_shows_delete_button_only_for_owner(
    client,
    create_team,
    create_user,
    login,
    ...
    owner_login = login("team_delete_owner", "password123")
    ...
    member_login = login("team_delete_member", "password123")
...
def test_team_delete_removes_team_and_related_projects(
    app,
    client,
    create_project,
    create_task,
    ...
    task = create_task(owner, project=project, title="Cascade Task")

    login_response = login("cascade_owner", "password123")
    assert login_response.status_code == 302

    response = client.post(
        f"/todo/teams/{team.id}/delete",
        data={},
        follow_redirects=False,
    )
...
def test_team_delete_forbidden_for_member_and_logs_warning(
    app,
    client,
    create_project,
    create_team,
    ...
    project = create_project(owner, team=team, name="Still Here")

    login_response = login("delete_forbidden_member", "password123")
    assert login_response.status_code == 302
    ...
    assert response.status_code == 403
    assert "team delete forbidden:" in caplog.text
    assert "reason=not_owner" in caplog.text

    with app.app_context():
        assert db.session.get(Project, project.id) is not None
        assert db.session.get(type(team), team.id) is not None


def test_team_delete_blocks_outsider_and_logs_warning(
    app,
    client,
    create_team,
    create_user,
    ...
    login_response = login("delete_outsider_user", "password123")
    assert login_response.status_code == 302
    ...
    assert response.status_code == 403
    assert "team delete forbidden:" in caplog.text
    assert "reason=not_member" in caplog.text
